...
export function RatioCatalogueSelector({
  open,
  onClose,
  engagementId,
  practiceTier,
  ...
  useEffect(() => {
    if (!open) return;
    (async () => {
      const { data } = await supabase.from('bi_ratio_definitions').select('code,name,category,min_tier,default_visual').eq('is_active', true);
  ...
  const toggle = async (code: string, select: boolean) => {
    const action = select ? 'add_ratio' : 'remove_ratio';
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40">
      <div className="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[85vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
          <div>
            <h3 className="font-semibold text-slate-800">Ratio catalogue</h3>
            <p className="text-xs text-slate-500">
              Tier {level}: up to {caps.maxRatios} ratios
            </p>
          </div>
              ...
              <div className="space-y-2">
                {rows.map((d) => {
                  ...
                  return (
                    <label
                      key={d.code}
                      className={`flex items-start gap-3 p-2 rounded-lg border ${ok ? 'border-slate-200' : 'border-slate-100 opacity-50'}`}
                    >
                      <input
                        type="checkbox"
                        checked={sel}
