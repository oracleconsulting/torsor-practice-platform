-- =============================================================================
-- Option C: Hybrid Opportunity Surfacing — RLS, Claude Partridge visibility, Page 5 fix
-- Run in Supabase SQL Editor. Engagement ID for Claude: 12d7650b-3f84-43a1-8503-5875f0dd2e6f
-- =============================================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Table discovery_opportunities does not exist — skip RLS';
    RETURN;
  END IF;
  IF NOT (SELECT relrowsecurity FROM pg_class WHERE relname = 'discovery_opportunities') THEN
...
-- VERIFY
-- -----------------------------------------------------------------------------
SELECT policyname, cmd FROM pg_policies WHERE tablename = 'discovery_opportunities';
