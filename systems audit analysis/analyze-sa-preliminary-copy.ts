...
function buildPreliminaryPrompt(
  ...
  const stage1Entries = Object.entries(raw).filter(([k]) => !k.endsWith('_context'));
  const stage1Text = stage1Entries
    .map(([k, v]) => {
      const context = raw[`${k}_context`];
      return `${k}: ${TRUNCATE(typeof v === 'string' ? v : JSON.stringify(v), MAX_FIELD)}${context ? `\n  → Context: "${TRUNCATE(String(context), 300)}"` : ''}`;
    })
    .join('\n');
  ...
  const stage2Text = systemsSlice
    .map((s: any) => {
      ...
      const noteEntries = Object.entries(notes)
        .map(([f, note]) => `    → ${f} note: "${TRUNCATE(String(note), 200)}"`)
        .join('\n');
      return `- ${s.system_name} (${s.category_code}): criticality=${s.criticality}, cost=£${s.monthly_cost || 0}/mo, integration=${s.integration_method || 'none'}, issues="${TRUNCATE(s.known_issues || '', 200)}"${noteEntries ? '\n' + noteEntries : ''}`;
    })
    .join('\n');
  ...
  const stage3Text = deepSlice
    .map((dd: any) => {
      ...
      const respEntries = Object.entries(responses).filter(([k]) => !k.endsWith('_context'));
      const respLines = respEntries
        .slice(0, 15)
        .map(([k, v]) => {
          const context = responses[`${k}_context`];
          return `  ${k}: ${TRUNCATE(JSON.stringify(v), 300)}${context ? `\n    → Context: "${TRUNCATE(String(context), 200)}"` : ''}`;
        });
