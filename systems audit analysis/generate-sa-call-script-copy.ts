    ...
    try {
      ...
      if (isTruncated) {
        ...
        try {
          ...
          console.error('[SA Call Script] JSON repair failed. Raw length:', cleaned.length, 'Preview (last 200):', cleaned.slice(-200));
          throw new Error('Script response was truncated or invalid JSON. Try again or reduce the number of gaps.');
        }
      } else {
        throw parseErr;
      }
