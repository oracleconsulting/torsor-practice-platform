...
export default function SystemsMapSection({ systemsMaps, facts }: { systemsMaps: any; facts: any }) {
  ...
  return (
    <div style={{ background: 'linear-gradient(145deg, #020617 0%, #0f172a 40%, #1e1b4b08 100%)', fontFamily: "'DM Sans', sans-serif", color: '#e2e8f0', padding: '24px', borderRadius: '16px' }}>
      ...
      {activeMap > 0 && current.changes && current.changes.length > 0 && (() => {
        const changes: any[] = current.changes || [];
        let steps: { title: string; owner: string; detail: string; extra?: string }[] = [];
        if (activeMap === 1) {
          steps = changes.map((ch: any) => ({
          ...
          steps = changes.filter((ch: any) => ch.action === 'connected' || (ch.description || '').toLowerCase().includes('connect')).map((ch: any) => ({
            title: ch.system || ch.description || 'Connection',
            owner: 'Technical team or integration specialist',
            detail: ch.description || 'Configure middleware trigger and action flow. Build time: 8-12 hours including testing.',
            extra: (ch.description || '').toLowerCase().includes('zapier') || (ch.description || '').toLowerCase().includes('make')
              ? 'GDPR note: Data passes through US servers. Evaluate Power Automate for personal data.'
              : undefined,
          }));
        } else if (activeMap === 3) {
          const stages = current.implementationStages;
          if (stages && Array.isArray(stages) && stages.length > 0) {
            steps = stages.map((st: any, idx: number) => ({
            ...
            if (replaces.length > 0) steps.push({ title: `Phase 1: Replacements (${replaces.length})`, owner: 'Project lead + vendor', detail: replaces.map((c: any) => c.system || c.description).filter(Boolean).join(', ') });
            if (reconfigs.length > 0) steps.push({ title: `Phase 2: Reconfigurations (${reconfigs.length})`, owner: 'Internal IT / vendor support', detail: reconfigs.map((c: any) => c.system || c.description).filter(Boolean).join(', ') });
            if (steps.length === 0) changes.forEach((ch: any, idx: number) => steps.push({ title: `Step ${idx + 1}: ${ch.system || ch.description || 'Change'}`, owner: 'Advisory team', detail: ch.description || ch.why || '' }));
          }
        }
        if (steps.length === 0) return null;
        return (
          <div style={{ marginTop: '16px', background: '#0a1628', border: '1px solid #1e293b', borderRadius: '12px', overflow: 'hidden' }}>
            <button onClick={() => setGuideOpen(!guideOpen)} style={{ width: '100%', padding: '14px 20px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', background: guideOpen ? '#22c55e10' : 'transparent', border: 'none', borderBottom: guideOpen ? '1px solid #1e293b' : 'none' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 600, color: '#22c55e', fontFamily: "'DM Sans', sans-serif" }}>Implementation Guide — {steps.length} step{steps.length !== 1 ? 's' : ''}</span>
            </button>
            {guideOpen && (
              <div style={{ padding: '4px 0' }}>
                {steps.map((step, i) => (
                  <div key={i} style={{ padding: '14px 20px', borderBottom: i < steps.length - 1 ? '1px solid #1e293b' : 'none' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                      <span style={{ width: '22px', height: '22px', borderRadius: '11px', background: '#22c55e20', color: '#22c55e', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700, flexShrink: 0, fontFamily: "'JetBrains Mono', monospace" }}>{i + 1}</span>
                      <span style={{ fontSize: '13px', fontWeight: 600, color: '#e2e8f0', fontFamily: "'DM Sans', sans-serif" }}>{step.title}</span>
                    </div>
                    <div style={{ marginLeft: '32px' }}>
                      <span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '4px', background: '#7c3aed15', color: '#a78bfa', border: '1px solid #7c3aed30', fontFamily: "'JetBrains Mono', monospace", fontWeight: 600 }}>{step.owner}</span>
                ...
                {activeMap === 3 && (
                  <div style={{ padding: '14px 20px', borderTop: '1px solid #1e293b', background: '#22c55e05' }}>
                    <p style={{ fontSize: '12px', color: '#94a3b8', lineHeight: '1.6', margin: 0, fontFamily: "'DM Sans', sans-serif" }}>
                      This implementation plan is designed to be self-contained. Your team can begin making these changes independently. For guided support through the transition, contact your advisory team about implementation packages.
                    </p>
                  </div>
                )}
              </div>
