...
-- ============================================================================
-- FIXED: Disabled audit triggers during table creation
-- ============================================================================

-- Temporarily disable audit triggers if they exist
DO $$ 
BEGIN
    -- Try to disable the audit trigger on assessment_questions if it exists
...
DROP TABLE IF EXISTS client_pattern_analysis CASCADE;
DROP TABLE IF EXISTS client_operational_context CASCADE;
DROP TABLE IF EXISTS client_financial_context CASCADE;
...
CREATE INDEX idx_client_financial_context_client ON client_financial_context(client_id);

-- Client operational context (things team knows/observes)
CREATE TABLE client_operational_context (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id uuid REFERENCES practice_members(id) ON DELETE CASCADE,
    practice_id uuid REFERENCES practices(id),
...
CREATE INDEX idx_client_operational_context_client ON client_operational_context(client_id);

-- Pattern analysis results (AI-generated insights)
CREATE TABLE client_pattern_analysis (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id uuid REFERENCES practice_members(id) ON DELETE CASCADE,
    practice_id uuid REFERENCES practices(id),
...
CREATE INDEX idx_client_pattern_analysis_client ON client_pattern_analysis(client_id);
...
DROP POLICY IF EXISTS "Team manages financial context" ON client_financial_context;
...
DROP POLICY IF EXISTS "Team manages operational context" ON client_operational_context;
...
DROP POLICY IF EXISTS "Team manages pattern analysis" ON client_pattern_analysis;
...
UNION ALL
SELECT 'Financial Context Table', CASE WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'client_financial_context') THEN 1 ELSE 0 END
UNION ALL
SELECT 'Operational Context Table', CASE WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'client_operational_context') THEN 1 ELSE 0 END
UNION ALL
SELECT 'Pattern Analysis Table', CASE WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'client_pattern_analysis') THEN 1 ELSE 0 END;
