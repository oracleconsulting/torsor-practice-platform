-- Update Tom's assessments with correct column name (client_id not client_member_id)
...
BEGIN
    ...
    RAISE NOTICE 'Tom id: %, practice_id: %', tom_id, tom_practice_id;

    -- Delete any existing assessments for Tom (clean slate)
    DELETE FROM client_assessments WHERE client_id = tom_id;
    RAISE NOTICE 'Cleared existing assessments for Tom';

    -- Insert Part 1: Life Design (15 questions)
    ...
    RAISE NOTICE 'Inserted Part 1 assessment';

    -- Insert Part 2: Business Deep Dive (72 questions across 12 sections)
    -- Tom answered approximately 50 questions from the original assessment
    INSERT INTO client_assessments (
        practice_id,
        client_id,
        assessment_type,
        ...
        '{
            ...
            "core_systems": ["Accounting (e.g. Xero, Sage)"],
            ...
            "main_customer_segment": "Schools, universities, rowing clubs, crossfit gyms, and home users across the UK",
            "customer_rating": 10,
            ...
            "supplier_relationships": "Decent partnerships",
            "procurement_management": "Gut feel",
            "competitive_position": "Clear leader",
            "ma_partnership_thoughts": "Might sell someday",
            "rating_financial_clarity": 4,
            "rating_customer_happiness": 9,
            "rating_team_performance": 6,
            "rating_operational_efficiency": 5,
            "rating_growth_momentum": 3,
            "rating_risk_management": 4,
            "rating_innovation_pipeline": 2,
            "rating_tech_systems": 3,
            "three_experts_needed": "Operations manager, Sales/BD person, Finance controller",
            "ten_hours_back": "Stop being the primary service technician",
            ...
            "help_fears": "They won''t understand us",
            "monday_frustration": "Waking up knowing I have to be on the road fixing machines instead of working ON the business",
            "magic_away_task": "Being the primary technician and having to fix machines personally",
            "family_feedback": "That I need to work less and be more present, and stop always being on call",
            "business_secret": "How dependent everything is on me being available 24/7",
            "secret_pride": "Building a business that''s the clear market leader from nothing, while being self-taught",
            "two_week_break_impact": "Customer service, job scheduling, quality control - basically everything",
            "overlooked_strength": "Our reputation and customer loyalty - we''ve never lost a major client",
            "unasked_help": "Other successful small business owners - I''m embarrassed to admit I don''t know what I''m doing",
            "last_excitement": "When we secured a big multi-year contract with a university chain - felt like real progress"
        }'::jsonb,
        'completed',
        100,
        12,
    ...
    RAISE NOTICE 'Inserted Part 2 assessment';

    -- Insert Part 3: Hidden Value Audit (32 questions across 6 sections)
    INSERT INTO client_assessments (
        practice_id,
        client_id,
        assessment_type,
        ...
        '{
            ...
            "competitive_moat": ["Exclusive contracts or partnerships", "Deep customer relationships", "Specialized expertise/talent", "Brand recognition and trust"],
    ...
    RAISE NOTICE 'Inserted Part 3 assessment';
...
SELECT 
    ...
    ca.total_sections,
    ca.completed_at,
    jsonb_object_keys(ca.responses) as response_keys_count
FROM client_assessments ca
JOIN practice_members pm ON pm.id = ca.client_id
...
SELECT 
    ca.assessment_type,
    jsonb_array_length(jsonb_path_query_array(ca.responses, '$.*')) as total_responses
FROM client_assessments ca
JOIN practice_members pm ON pm.id = ca.client_id
WHERE pm.email = 'tom@rowgear.com'
ORDER BY ca.assessment_type;
