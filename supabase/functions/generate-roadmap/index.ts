...
serve(async (req) => {
  ...
  try {
    ...
    // Cross-service enrichment (graceful degradation)
    ...
    try {
      ...
      if (enrichedContext?.hasEnrichment) {
        console.log('[generate-roadmap] Cross-service enrichment available');
      }
    } catch (err) {
      console.warn('[generate-roadmap] Enrichment not available:', err);
    }
...
`
    ...
    let fiveYearVision;
    try {
      const visionPrompt = buildVisionPrompt(context) + enrichmentBlock;
    ...
    let sixMonthShift;
    try {
      const shiftPrompt = buildShiftPrompt(context, fiveYearVision) + enrichmentBlock;
    ...
    try {
      ...
      // INJECT ADVISOR CONTEXT AND ENRICHMENT AT TOP OF SPRINT PROMPT
      if (advisorContextSection || enrichmentBlock) {
        sprintPrompt = (advisorContextSection || '') + (enrichmentBlock ? '\n\n' + enrichmentBlock : '') + '\n\n' + sprintPrompt;
        if (advisorContextSection) console.log('Injected advisor context into sprint prompt');
        if (enrichmentBlock) console.log('Injected cross-service enrichment into sprint prompt');
      }
