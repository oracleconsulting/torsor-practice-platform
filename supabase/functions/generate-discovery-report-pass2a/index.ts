// ============================================================================
// DISCOVERY REPORT - PASS 2A: STRUCTURAL DRAFT (Sonnet)
// ============================================================================
// Generates complete 5-page report with correct schema, numbers, and structure.
// Pass 2B (Opus) enhances narrative sections. This pass focuses on accuracy.
...
const SERVICE_REGISTRY: Record<string, ServiceDefinition> = {
  ...
  systems_audit: { code: 'systems_audit', name: 'Systems & Process Audit', displayName: 'Systems & Process Audit', category: 'foundation', outcome: "You'll See What's Actually Running Your Business", description: 'A comprehensive review of your operational systems, processes, and dependencies. We map what\'s documented, what\'s assumed, what only lives in one person\'s head, and where the single points of failure are.', keywords: ['systems', 'process', 'efficiency', 'automation', 'audit', 'delegation', 'bottleneck', 'dependency'], tiers: [{ name: 'The Map', tagline: "See What's Actually Running Your Business", pricingModel: 'fixed', price: 2000, priceFormatted: '£2,000', period: 'one-off', periodLabel: '', examplePdfUrl: '/storage/service-examples/systems-audit-tier1.pdf', showInPopup: true, popupCtaLabel: 'View Example' }, { name: 'The Blueprint', tagline: 'See It, Fix It, Hand It Over', pricingModel: 'fixed', price: 4500, priceFormatted: '£4,500', period: 'one-off', periodLabel: '', examplePdfUrl: '/storage/service-examples/systems-audit-tier2.pdf', showInPopup: true, popupCtaLabel: 'View Example' }], defaultTierIndex: 0, isActive: true },
...
// Pass 2A: Sonnet for structure & accuracy (narrative enhanced by Pass 2B)
const PASS2A_MODEL = 'anthropic/claude-sonnet-4';
...
// ============================================================================
// UNIVERSAL PAYROLL FIGURE ENFORCEMENT (object-walking, JSON-safe)
// Walks parsed objects field-by-field. Never serializes/deserializes JSON.
// ============================================================================
...
function fixPayrollInString(text: string, correctExcessK: number, correctMonthlyK: number, correctBenchmarkPct: number): { text: string; changed: boolean; count: number } {
  ...
  // PROTECT: strings about returns, valuations, CoI — only fix benchmark %
  ...
  if (hasProtectedContext && !hasPayrollContext) {
    result = result.replace(/staff costs at ([\d.]+)% vs the? (\d+)% benchmark/gi, (_match: string, _actual: string, bench: string) => {
      if (parseInt(bench) !== correctBenchmarkPct) {
        count++;
        return _match.replace(`${bench}%`, `${correctBenchmarkPct}%`);
      }
      return _match;
    });
    return { text: result, changed: count > 0, count };
  ...
  // Annual payroll excess patterns
  ...
  // Monthly payroll patterns
  const monthlyPatterns = [
    /£(\d{1,3})k\s*walks?\s*out/gi,
    /£(\d{1,3})k\s*a\s*month/gi,
    /£(\d{1,3})k\/month/gi,
    /£(\d{1,3})k\s*every\s*month/gi,
  ];
  ...
  for (const pattern of monthlyPatterns) {
    result = result.replace(pattern, (match: string, amount: string) => {
      ...
      if (num > correctMonthlyK * 2 && num < correctMonthlyK * 6) {
        count++;
  ...
  result = result.replace(/the\s+(\d{2,3})%\s+benchmark/gi, (match: string, pct: string) => {
    ...
    if (num >= 20 && num <= 55) { count++; return match.replace(`${num}%`, `${correctBenchmarkPct}%`); }
    return match;
  });
  ...
  if (Array.isArray(obj)) {
    for (let i = 0; i < obj.length; i++) {
      if (typeof obj[i] === 'string') {
        const r = fixPayrollInString(obj[i], correctExcessK, correctMonthlyK, correctBenchmarkPct);
        if (r.changed) { obj[i] = r.text; replacements += r.count; }
      } else {
        replacements += enforcePayrollInObject(obj[i], correctExcessK, correctMonthlyK, correctBenchmarkPct);
      }
    }
    return replacements;
  }
  ...
  if (typeof obj === 'object') {
    for (const key of Object.keys(obj)) {
      if (typeof obj[key] === 'string') {
        const r = fixPayrollInString(obj[key], correctExcessK, correctMonthlyK, correctBenchmarkPct);
        if (r.changed) { obj[key] = r.text; replacements += r.count; }
      } else {
        replacements += enforcePayrollInObject(obj[key], correctExcessK, correctMonthlyK, correctBenchmarkPct);
      }
    }
  }
...
`;
    ...
    console.log(`[Pass2A] Building context notes section from ${contextNotes?.length || 0} notes`);
...
============================================================================
` : '';
    console.log(`[Pass2A] Context notes section length: ${contextNotesSection.length} chars`);
...
============================================================================
WRITING INSTRUCTIONS (Pass 2A — Structure & Accuracy)
============================================================================

You are generating the STRUCTURAL DRAFT of this report. Focus on:
1. Correct JSON schema — every field must be present and correctly typed
2. Accurate financial figures — use ONLY the pre-calculated numbers provided
3. Correct service names and prices — from the price constraints above
4. Client's actual quotes — use their exact words, do not paraphrase
5. Logical gap analysis — categories, priorities, and financial impacts must match the data

For narrative fields (visionVerbatim, openingLine, feelsLike, personalCost,
realReturn, theAsk, closingLine, urgencyAnchor), write competent first drafts.
These will be enhanced by a narrative specialist in the next pass. Focus on
getting the FACTS and STRUCTURE right rather than literary quality.
...
- To describe the client working alone, say "the strategic burden sits with you" or use their actual words.
${contextNotesSection}
${financialDataSection}
${financialHealthSection}
${mandatoryPhrasesSection}
...
- [ ] meta.readyForClient correctly reflects data completeness`;
    ...
    const llmResponse = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.2,  // Lower for structural accuracy
        max_tokens: 14000  // Slightly less needed without verbose narrative
      })
    });
    ...
    // Pass 2A: Save as 'generated' — Pass 2B will set 'published' after narrative enhancement
    const reportStatus = 'generated';
    const engagementStatus = 'pass2_complete';
    ...
    // ====================================================================
    // UNIVERSAL PAYROLL FIGURE ENFORCEMENT — before save (object-walking)
    // ====================================================================
    ...
    console.log('[Pass2A] ✅ Database update successful');
    // Pass 2B will handle headline quality
