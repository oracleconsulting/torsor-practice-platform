...
-- 2. Automatic CPD regeneration when knowledge base updates
-- 3. Notification system for new CPD material
...
ALTER TABLE cpd_recommendations 
  ...
  ADD COLUMN IF NOT EXISTS resource_type VARCHAR(50) CHECK (resource_type IN ('internal', 'external', 'both', 'none'));

CREATE INDEX IF NOT EXISTS idx_cpd_recommendations_knowledge_doc ON cpd_recommendations(linked_knowledge_doc_id);
CREATE INDEX IF NOT EXISTS idx_cpd_recommendations_external_resource ON cpd_recommendations(linked_external_resource_id);
...
BEGIN
  ...
  LOOP
    -- Call the regeneration function for each member
    -- This will be called from the application layer
    -- We just create notifications here
...
-- =====================================================
-- 12. INITIAL DATA POPULATION (OPTIONAL)
-- =====================================================
-- This can be uncommented to send notifications about existing resources
-- PERFORM auto_regenerate_cpd_recommendations_for_all();
