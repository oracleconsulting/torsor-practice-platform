-- COPY THIS ENTIRE SQL INTO SUPABASE
-- Creates 3 practice members if needed, then populates all skills
...
BEGIN
    ...
    -- Get or create practice_id
    SELECT id INTO practice_uuid FROM practices LIMIT 1;
    IF practice_uuid IS NULL THEN
        RAISE EXCEPTION 'No practice found. Create a practice first.';
    END IF;
    RAISE NOTICE 'Using practice: %', practice_uuid;
    
    -- Get existing members
    FOR member_record IN 
        SELECT id FROM practice_members WHERE practice_id = practice_uuid ORDER BY id LIMIT 3
    LOOP 
        member_ids := array_append(member_ids, member_record.id); 
        RAISE NOTICE 'Found existing member: %', member_record.id;
    END LOOP;
    
    -- Create additional members if needed
    WHILE array_length(member_ids, 1) < 3 LOOP
        INSERT INTO practice_members (practice_id, user_id, role, is_active)
        VALUES (practice_uuid, gen_random_uuid(), 'member', true)
        RETURNING id INTO new_member_id;
        
        member_ids := array_append(member_ids, new_member_id);
        RAISE NOTICE 'Created new member: %', new_member_id;
    END LOOP;
    
    RAISE NOTICE 'Have % members total', array_length(member_ids, 1);
